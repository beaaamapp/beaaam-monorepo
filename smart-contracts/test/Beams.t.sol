...
contract AssertMinAmtPerSec is Test, Beams {
    constructor(
        uint32 cycleSecs,
        uint160 expectedMinAmtPerSec
    ) Beams(cycleSecs, 0) {
        string memory assertMessage = string.concat(
            "Invalid minAmtPerSec for cycleSecs ",
            vm.toString(cycleSecs)
        );
...
contract BeamsTest is Test, PseudoRandomUtils, Beams {
    ...
    bytes internal constant ERROR_INVALID_DRIPS_LIST =
        "Invalid current beams list";
    bytes internal constant ERROR_TIMESTAMP_EARLY =
        "Timestamp before the last update";
    bytes internal constant ERROR_HISTORY_INVALID = "Invalid beams history";
    bytes internal constant ERROR_HISTORY_UNCLEAR =
        "Entry with hash and receivers";

    // Keys are assetId and userId
    mapping(uint256 => mapping(uint256 => BeamsReceiver[]))
        internal currReceiversStore;
    ...
    function recv(
        ...
        uint256 amtPerSecFull = amtPerSec *
            Beams._AMT_PER_SEC_MULTIPLIER +
            amtPerSecFrac;
        BeamsConfig config = BeamsConfigImpl.create(
            uint32(beamId),
            uint160(amtPerSecFull),
            uint32(start),
            uint32(duration)
        );
    ...
    function genRandomRecv(
        ...
        for (uint256 i = 0; i < amountReceiver; i++) {
            ...
            uint256 amtPerSec = _minAmtPerSec +
                random(maxAmtPerSec - _minAmtPerSec);
    ...
    function hist(
        ...
        uint32 updateTime,
        uint32 maxEnd
    ) internal pure returns (BeamsHistory[] memory history) {
    ...
    function hist(
        uint256 userId
    ) internal returns (BeamsHistory[] memory history) {
        BeamsReceiver[] memory receivers = loadCurrReceivers(userId);
        (, , uint32 updateTime, , uint32 maxEnd) = Beams._beamsState(
            userId,
            assetId
        );
    ...
    function histSkip(
        uint256 userId
    ) internal view returns (BeamsHistory[] memory history) {
        (bytes32 beamsHash, , uint32 updateTime, , uint32 maxEnd) = Beams
            ._beamsState(userId, assetId);
        return histSkip(beamsHash, updateTime, maxEnd);
    }

    function hist(
        BeamsHistory[] memory history,
        uint256 userId
    ) internal returns (BeamsHistory[] memory) {
    ...
    function histSkip(
        BeamsHistory[] memory history,
        uint256 userId
    ) internal view returns (BeamsHistory[] memory) {
        return hist(history, histSkip(userId));
    }

    function hist(
        BeamsHistory[] memory history1,
        BeamsHistory[] memory history2
    ) internal pure returns (BeamsHistory[] memory history) {
    ...
    function setBeams(
        ...
        assertEq(
            maxEnd,
            block.timestamp + expectedMaxEndFromNow,
            "Invalid max end"
        );
    }
    ...
    function assertBalanceAt(
        ...
        uint128 balance = Beams._balanceAt(
            userId,
            assetId,
            loadCurrReceivers(userId),
            uint32(timestamp)
    ...
    function receiveBeams(uint256 userId, uint128 expectedAmt) internal {
        uint128 actualAmt = Beams._receiveBeams(
            userId,
            assetId,
            type(uint32).max
        );
    ...
    function assertReceiveBeamsResult(
        ...
        (uint128 actualAmt, , , , ) = Beams._receiveBeamsResult(
            userId,
            assetId,
            type(uint32).max
        );
    ...
    function assertReceiveBeamsResult(
        ...
        (uint128 actualAmt, uint32 actualCycles, , , ) = Beams
            ._receiveBeamsResult(userId, assetId, maxCycles);
        assertEq(actualAmt, expectedAmt, "Invalid receivable amount");
        assertEq(
            actualCycles,
            expectedCycles,
            "Invalid receivable beams cycles"
        );
    }
    ...
    function squeezeBeams(
        ...
        (uint128 amtBefore, , , , ) = Beams._squeezeBeamsResult(
            userId,
            assetId,
            senderId,
            historyHash,
        ...
        assertEq(
            amtBefore,
            expectedAmt,
            "Invalid squeezable amount before squeezing"
        );

        uint128 amt = Beams._squeezeBeams(
            userId,
            assetId,
            senderId,
            historyHash,
        ...
        (uint128 amtAfter, , , , ) = Beams._squeezeBeamsResult(
            userId,
            assetId,
            senderId,
            historyHash,
    ...
    function assertSqueezeBeamsReverts(
        ...
        vm.expectRevert(expectedReason);
        this.squeezeBeamsResultExternal(
            userId,
            senderId,
            historyHash,
            beamsHistory
    ...
    function testBeamsToTwoReceivers() public {
        setBeams(
            ...
            0,
            100,
            recv(recv(receiver1, 1), recv(receiver2, 1)),
            50
        );
        skip(14);
    ...
    function testBeamsWithStartInThePastAndDurationIntoFuture() public {
        ...
        setBeams(
            ...
            0,
            3,
            recv(receiver, 1, block.timestamp - 5, 8),
            maxEndMax()
        );
        skip(3);
        assertBalance(sender, 0);
    ...
    function testBeamsWithStartAfterFundsRunOut() public {
        setBeams(
            ...
            recv(
                recv(receiver1, 1),
                recv(receiver2, 2, block.timestamp + 5, 0)
            ),
            4
        );
        skip(6);
    ...
    function testBeamsWithStartInTheFutureCycleCanBeMovedToAnEarlierOne()
        public
    {
        setBeams(
            sender,
            0,
            1,
            recv(receiver, 1, block.timestamp + _cycleSecs, 0),
            _cycleSecs + 1
        );
    ...
    function testDoesNotCollectCyclesBeforeFirstBeam() public {
        ...
        setBeams(
            ...
            0,
            1,
            recv(receiver, 1, block.timestamp + _cycleSecs * 2, 0),
            _cycleSecs * 2 + 1
        );
    ...
    function testAllowsBeamsConfigurationWithOverflowingTotalAmtPerSec()
        public
    {
        setBeams(
            sender,
    ...
    function testAllowsBeamsConfigurationWithOverflowingAmtPerCycle() public {
        ...
        uint128 amtPerSec = (uint128(type(int128).max) / _cycleSecs / 1000) *
            2345;
    ...
    function testAllowsBeamsConfigurationWithOverflowingAmtPerCycleAcrossCycleBoundaries()
        public
    {
        // amtPerSec is valid, but amtPerCycle is over 2 times higher than int128.max.
        // The multiplier is chosen to prevent the amounts from being "clean" binary numbers
    ...
    function testAllowsBeamsConfigurationWithOverflowingAmtDeltas() public {
        ...
        setBeams(
            ...
            0,
            amtPerSec,
            recv(receiver, amtPerSec, _cycleSecs / 2, 0),
            1
        );
        skipToCycleEnd();
    ...
    function testAllowsChangingReceiversWhileBeamping() public {
        setBeams(
            ...
            0,
            100,
            recv(recv(receiver1, 6), recv(receiver2, 6)),
            8
        );
        skip(3);
        setBeams(
            ...
            64,
            64,
            recv(recv(receiver1, 4), recv(receiver2, 8)),
            5
        );
        skip(4);
    ...
    function testAllowsRemovingReceiversWhileBeamping() public {
        setBeams(
            ...
            0,
            100,
            recv(recv(receiver1, 5), recv(receiver2, 5)),
            10
        );
        skip(3);
    ...
    function testFractionsAreClearedOnCycleBoundary() public {
        ...
        setBeams(
            ...
            0,
            3,
            recv(receiver, 0, Beams._AMT_PER_SEC_MULTIPLIER / 4 + 1),
            17
        );
        skipToCycleEnd();
        assertBalance(sender, 1);
    ...
    function testFractionsAreAppliedOnCycleSecondsWhenTheyAddUpToWholeUnits()
        public
    {
        assertEq(_cycleSecs, 10, "Unexpected cycle length");
        // Rate of 0.25 per second
    ...
    function testFractionsAreAppliedRegardlessOfStartTime() public {
        ...
        setBeams(
            ...
            0,
            1,
            recv(receiver, 0, (Beams._AMT_PER_SEC_MULTIPLIER / 10) * 4 + 1),
            4
        );
    ...
    function testBeamsWithFractionsCanBeSeamlesslyToppedUp() public {
        assertEq(_cycleSecs, 10, "Unexpected cycle length");
        // Rate of 0.25 per second
        BeamsReceiver[] memory receivers = recv(
            receiver,
            0,
            Beams._AMT_PER_SEC_MULTIPLIER / 4 + 1
        );
        // Full units are beamped on cycle timestamps 3 and 7
        ...
        setBeams(
            ...
            recv(
                ...
                recv(
                    receiver2,
                    0,
                    (Beams._AMT_PER_SEC_MULTIPLIER / 100 + 1) * 33
                )
            ),
            13
        );
    ...
    function testFractionsDoNotCumulateOnReceiver() public {
        ...
        setBeams(
            ...
            0,
            7,
            recv(receiver, 0, (Beams._AMT_PER_SEC_MULTIPLIER / 100 + 1) * 66),
            13
        );
        skipToCycleEnd();
    ...
    function testRejectsTooLowAmtPerSecReceivers() public {
        assertSetBeamsReverts(
            ...
            0,
            0,
            recv(receiver, 0, _minAmtPerSec - 1),
            "Beams receiver amtPerSec too low"
        );
    }
    ...
    function testSenderCanBeamToThemselves() public {
        ...
        setBeams(
            ...
            0,
            amt,
            recv(recv(sender, 1), recv(receiver, 2)),
            _cycleSecs
        );
        skipToCycleEnd();
    ...
    function testFuzzBeamsReceiver(bytes32 seed) public {
        ...
        uint128 maxCosts = amountReceivers *
            uint128(maxAmtPerSec / _AMT_PER_SEC_MULTIPLIER) *
            maxDuration;
        emit log_named_uint("topUp", maxCosts);
        uint128 maxAllBeamsFinished = maxStart + maxDuration;

        BeamsReceiver[] memory receivers = genRandomRecv(
            amountReceivers,
            maxAmtPerSec,
            maxStart,
            maxDuration
    ...
    function sanitizeReceivers(
        ...
        receivers = new BeamsReceiver[](
            bound(receiversLengthRaw, 0, receiversRaw.length)
        );
        for (uint256 i = 0; i < receivers.length; i++) {
        ...
        for (uint32 i = 0; i < receivers.length; i++) {
            ...
            receivers[i].config = BeamsConfigImpl.create(
                i,
                amtPerSec,
                cfg.start(),
                cfg.duration()
    ...
    function sanitizeSenders(
        ...
        for (uint32 i = 0; i < sendersLen; i++) {
            ...
            uint160 amtPerSec = cfg.amtPerSec();
            if (amtPerSec < _minAmtPerSec) amtPerSec = _minAmtPerSec;
            senders[i].receivers[0].config = BeamsConfigImpl.create(
                i,
                amtPerSec,
                cfg.start(),
                cfg.duration()
        ...
        for (uint256 i = 0; i < sendersLen; i++) {
            ...
            uint256 newUsedBalance = (usedBalanceWeight * balance) /
                totalBalanceWeight;
    ...
    function sanitizeBeamTime(
        uint256 beamTimeRaw,
        uint256 maxCycles
    ) internal view returns (uint256 beamTime) {
        return bound(beamTimeRaw, 0, _cycleSecs * maxCycles);
    }

    function sanitizeBeamBalance(
        uint256 balanceRaw
    ...
    function testFundsBeampedToReceiversAddUp(
        ...
        BeamsReceiver[] memory receivers = sanitizeReceivers(
            receiversRaw,
            receiversLengthRaw
        );
        Beams._setBeams(
        ...
        for (uint256 i = 0; i < receivers.length; i++) {
            balanceAfter += Beams._receiveBeams(
                receivers[i].userId,
                asset,
                type(uint32).max
            );
    ...
    function testFundsBeampedToReceiversAddUpAfterBeamsUpdate(
        ...
        uint128 balanceBefore = sanitizeBeamBalance(balanceRaw);
        BeamsReceiver[] memory receivers1 = sanitizeReceivers(
            receiversRaw1,
            receiversLengthRaw1
        );
        Beams._setBeams(
        ...
        BeamsReceiver[] memory receivers2 = sanitizeReceivers(
            receiversRaw2,
            receiversLengthRaw2
        );
        int128 realBalanceDelta = Beams._setBeams(
    ...
    function testFundsBeampedFromSendersAddUp(
        ...
        uint128 balanceBefore = sanitizeBeamBalance(balanceRaw);
        Sender[] memory senders = sanitizeSenders(
            receiverId,
            balanceBefore,
            sendersRaw,
            sendersLenRaw
    ...
    function testSqueezeBeamsRevertsWhenHistoryEntryContainsReceiversAndHash()
        public
    {
        uint128 amt = _cycleSecs;
        setBeams(sender, 0, amt, recv(receiver, 1), _cycleSecs);
    ...
    function testBeamsWithStartAndDurationCanBeSqueezed() public {
        setBeams(
            ...
            0,
            10,
            recv(receiver, 1, block.timestamp + 2, 2),
            maxEndMax()
        );
        skip(5);
        squeezeBeams(receiver, sender, hist(sender), 2);
    ...
    function testBeamsWithManyReceiversCanBeSqueezed() public {
        setBeams(
            ...
            0,
            14,
            recv(recv(receiver1, 1), recv(receiver2, 2), recv(receiver3, 4)),
            2
        );
        skip(1);
    ...
    function testUnsqueezedHistoryEntriesFromBeforeLastSqueezeCanBeSqueezed()
        public
    {
        setBeams(sender, 0, 9, recv(receiver, 1), 9);
    ...
    function testLastSqueezedForConfigurationSetInPastCycleIsKeptAfterUpdatingBeams()
        public
    {
        setBeams(sender, 0, 2, recv(receiver, 2), 1);
        BeamsHistory[] memory history = hist(sender);

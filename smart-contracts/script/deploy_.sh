...
ADMIN=${ADMIN:-$WALLET}
BEAMS_HUB_ADMIN=$(cast --to-checksum-address "${BEAMS_HUB_ADMIN:-$ADMIN}")
...
BEAMS_HUB_CYCLE_SECS=${BEAMS_HUB_CYCLE_SECS:-$(( 7 * 24 * 60 * 60 ))} # 1 week
...
echo "BeamsHub cycle seconds:       $BEAMS_HUB_CYCLE_SECS"
echo "BeamsHub admin:               $BEAMS_HUB_ADMIN"
...
tee "$DEPLOYMENT_JSON" <<EOF
{
    ...
    "Deployer":                     "$DEPLOYER",
    "BeamsHub":                     "$(deployment_detail beamsHub address)",
    "BeamsHub cycle seconds":       "$(deployment_detail beamsHubCycleSecs uint32)",
    "BeamsHub logic":               "$(deployment_detail beamsHubLogic address)",
    "BeamsHub admin":               "$(deployment_detail beamsHubAdmin address)",
...
if [ -n "$ETHERSCAN_API_KEY" ]; then
    ...
    verifyWithProxy beamsHub src/BeamsHub.sol:BeamsHub

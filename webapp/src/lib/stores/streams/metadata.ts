...
import seperateBeamsSetEvents from './methods/separate-beams-set-events';
import buildAssetConfigs from './methods/build-asset-configs';
import { getAddressDriverClient, getSubgraphClient } from '$lib/utils/get-beams-clients';
import mapFilterUndefined from '$lib/utils/map-filter-undefined';
import { reconcileBeamsSetReceivers } from './methods/reconcile-beams-set-receivers';
...
export const beamsConfigSchema = z.object({
  raw: z.string(),
  beamId: z.string(),
  ...
  /**
   * If undefined, the block timestamp from the initial setBeams event
...
export const streamMetadataSchema = z.object({
  id: z.string(),
  initialBeamsConfig: beamsConfigSchema,
  receiver: userSchema,
  archived: z.boolean(),
    ...
    assetConfigs: mapFilterUndefined(forAccount.assetConfigs, (assetConfig) => ({
      ...
      streams: mapFilterUndefined(assetConfig.streams, (stream) => {
        ...
        return {
          id: stream.id,
          initialBeamsConfig: {
            beamId: stream.beamsConfig.beamId,
            raw: stream.beamsConfig.raw.toString(),
            startTimestamp: Math.floor((stream.beamsConfig.startDate?.getTime() || 0) / 1000),
            durationSeconds: stream.beamsConfig.durationSeconds || 0,
...
export async function fetchAccount(userId: UserId): Promise<Account> {
  ...
  const beamsSetEvents = await subgraphClient.getBeamsSetEventsByUserId(userId);
  const beamsSetEventsWithFullReceivers = reconcileBeamsSetReceivers(beamsSetEvents);
  const beamsSetEventsByTokenAddress = seperateBeamsSetEvents(beamsSetEventsWithFullReceivers);

  const assetConfigs = buildAssetConfigs(userId, data, beamsSetEventsByTokenAddress);

import type * as radicleBeams from 'beaaams-backend';
...
vi.mock('beaaams-backend', async () => ({
  ...((await vi.importActual('beaaams-backend')) as typeof radicleBeams),
  BeamsSubgraphClient: {
    create: () => ({
      getBeamsSetEventsByUserId: () => MOCK_BEAMS_SET_EVENTS,
...
describe('metadata.ts', () => {
  describe('fetchAccount', () => {
    ...
    it('builds the correct assetConfig history for RAD', async () => {
      ...
      According to mocked beams set events, there should be two active RAD streams.
      */
      ...
      According to mocked beams set events, one of the streams should be paused now.
      */
      ...
      According to mocked beams set events, the previously paused stream should be unpaused.
      */
    ...
    it('builds the correct assetConfig history for DAI', async () => {
      ...
      According to mocked beams set events, there should be one active DAI stream.
      */

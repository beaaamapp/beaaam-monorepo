...
export default (() => {
  ...
  /**
   * A store that includes beams user IDs and their current AccountFetchStatus. Only
  ...
  async function fetchAccount(userId: UserId): Promise<Account> {
    ...
    try {
      ...
      const beamsReceiverSeenEventForUser =
        await subgraphClient.getBeamsReceiverSeenEventsByReceiverId(userId);
      const accountsSendingToCurrentUser = beamsReceiverSeenEventForUser.reduce<string[]>(
        (acc, event) => {

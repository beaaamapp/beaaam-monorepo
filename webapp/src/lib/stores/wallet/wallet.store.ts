...
const walletStore = () => {
  ...
  async function connect(initializing = false, isSafeApp = false): Promise<void> {
    ...
    if (!_isNetworkSupported(await provider.getNetwork())) {
      const clearAdvisory = globalAdvisoryStore.add({
        ...
        description: 'Please switch your connected wallet to Base Goerli.',
        emoji: '🔌',
      });
  ...
  async function _setConnectedState(
    ...
    state.set({
      connected: true,
      address: accounts[0],
      beamsUserId: await (await AddressDriverClient.create(provider, signer)).getUserId(),
      provider,
      signer,

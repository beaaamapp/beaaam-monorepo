import { constants, type SqueezedBeamsEvent } from 'beaaams-backend';
...
import { getBeamsHubClient, getSubgraphClient } from '$lib/utils/get-beams-clients';
...
interface AccountBalances {
  receivable?: Amount[];
  splittable?: Amount[];
  squeezeHistory?: SqueezedBeamsEvent[];
...
export default (() => {
  ...
  async function updateSqueezeHistory(forUserId: string) {
    ...
    const squeezedEvents = (await subgraph.getSqueezedBeamsEventsByUserId(forUserId)).sort(
  ...
  /**
   * The balances store internally fetches information about the current BeamsHub cycle in order to
  ...
  async function updateCycle() {
    ...
    const cycleSecs = await beamsHubClient.cycleSecs();
  ...
  function getFullSqueezeHistory() {
    return Object.values(get(state).accounts).reduce<SqueezedBeamsEvent[]>((acc, account) => {

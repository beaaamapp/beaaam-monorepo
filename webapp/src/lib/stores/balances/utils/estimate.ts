...
import type { SqueezedBeamsEvent } from 'beaaams-backend';
...
export function estimateAccount(
  account: Account,
  currentCycle: Cycle,
  excludingSqueezes: SqueezedBeamsEvent[] = [],
): AccountEstimate {
...
function streamedByStream(
  ...
  // Undefined beamsConfig means the stream was paused.
  if (!receiver.beamsConfig) {
    return {
      streamed: 0n,
  ...
  const { durationSeconds, amountPerSecond, startDate } = receiver.beamsConfig;
  ...
  const squeezedAtBlockTimestamp = excludingSqueezes.find(
    (squeezeEvent) =>
      ...
      squeezeEvent.beamsHistoryHashes.includes(historyItem.historyHash),
  )?.blockTimestamp;

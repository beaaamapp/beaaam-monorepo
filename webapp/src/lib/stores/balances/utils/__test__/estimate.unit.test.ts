...
const mockStream = ({
  ...
  id: `${MOCK_USER.userId}-0x00-${beamId}`,
  sender: MOCK_USER,
  receiver: MOCK_USER,
  beamsConfig: {
    raw: Utils.BeamsReceiverConfiguration.toUint256({
      amountPerSec: amountPerSecond,
      start: BigInt(startTimestamp),
      duration: BigInt(durationSeconds),
      beamId: BigInt(beamId),
    }),
    amountPerSecond: {
      tokenAddress: '0x00',
...
const mockAssetConfigHistoryItem = ({
  ...
  streams: streams.map((stream) => ({
    streamId: stream.id,
    beamsConfig: stream.paused ? undefined : stream.beamsConfig,

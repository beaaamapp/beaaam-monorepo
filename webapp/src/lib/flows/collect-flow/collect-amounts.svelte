<script lang="ts">
  ...
  import { AddressDriverPresets, constants, type CollectedEvent } from 'beaaams-backend';
  ...
  $: cycle = $context.currentBeamsCycle ?? unreachable();
  ...
  $: incomingEstimatesBySender =
    $balancesStore &&
    balancesStore
      .getStreamEstimatesByReceiver('currentCycle', $wallet.beamsUserId ?? unreachable())
    ...
    transact(
      ...
      makeTransactPayload({
        before: async () => {
          ...
          const { BEAMS_HUB, ADDRESS_DRIVER } = getNetworkConfig();
...
<StepLayout>
  ...
  <div>
    ...
    <a
      ...
      target="_blank"
      rel="noreferrer"
      href="https://docs.beams.network/docs/the-beams-app/manage-funds/collect-earnings"
      >Learn more</a
    >
  </div>

...
import { getAddressDriverClient, getAddressDriverTxFactory } from '$lib/utils/get-beams-clients';
import mapFilterUndefined from '$lib/utils/map-filter-undefined';
import { constants } from 'ethers';
import { ERC20TxFactory } from 'beaaams-backend';
  ...
  transact(
    ...
    makeTransactPayload({
      before: async () => {
        ...
        const setBeamsPopulatedTx = await txFactory.setBeams(
          tokenAddress,
          currentReceivers,
          amountToTopUp,
          currentReceivers,
        ...
        delete setBeamsPopulatedTx.gasLimit;

        const setBeamsTx = client.setBeams(
          tokenAddress,
          currentReceivers,
          currentReceivers,
          address,
      ...
      transactions: ({ setBeamsTx, setBeamsPopulatedTx, approvePopulatedTx, needApproval }) =>
        // If the ERC-20 needs approval, we send a batch TX including the approval TX and setBeams.
        needApproval
          ? [
              {
              ...
              {
                transaction: setBeamsPopulatedTx,
                waitingSignatureMessage: {

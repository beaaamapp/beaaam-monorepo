<script lang="ts">
  ...
  import type { TextInputValidationState } from '$lib/components/text-input/TextInput';
  import TextInput from '$lib/components/text-input/text-input.svelte';
  import { constants } from 'beaaams-backend';
  ...
  $: estimate =
    $balances.accounts[String($wallet.beamsUserId) ?? unreachable()].tokens[$context.tokenAddress]
  ...
  function getAssetConfigHistory(beamsUserId: string, tokenAddress: string) {
    return (
      get(streams).accounts[beamsUserId].assetConfigs.find(
  ...
  function triggerWithdraw() {
    transact(
      ...
      makeTransactPayload({
        before: async () => {
          const { address, beamsUserId } = $wallet;
          assert(address && beamsUserId);
          ...
          const tx = addressDriverClient.setBeams(
